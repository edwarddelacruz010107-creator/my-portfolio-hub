...
class TestPasswordResetEnumerationProtection:
    ...
    def test_tenant_email_mismatch_returns_generic_error(self):
        """Email/contact_email mismatch must not reveal whether account exists."""
        from app.services.password_reset_service import initiate_tenant_reset

        fake_user = MagicMock()
        fake_user.tenant_id = 10
        fake_tenant = MagicMock()
        fake_tenant.contact_email = 'different@example.com'
        ...
        with patch('app.services.password_reset_service.User') as MockUser, \
            ...
            MockTenant.query.get.return_value = fake_tenant
            ok, msg = initiate_tenant_reset('submitted@example.com')
